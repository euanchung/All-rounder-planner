...
export default async function handler(req,res){try{
 ...
 const offset=Number(new URL(req.url,'https://local.invalid').searchParams.get('offset')||0);if(![0,1].includes(offset))return respond(res,400,{error:'오늘 또는 내일 급식만 조회할 수 있어요.'});const date=new Date(Date.now()+9*3600000+offset*86400000).toISOString().slice(0,10).replaceAll('-',''),key=school+':'+date;
 ...
 const text=meals.length?school+' · '+date+'\n'+meals.map(m=>m.MMEAL_SC_NM+'\n'+m.DDISH_NM.replace(/<br\s*\/?\s*>/gi,'\n').replace(/<[^>]+>/g,'')).join('\n\n')+'\n출처: 나이스 교육정보 개방 포털. 괄호의 알레르기 번호는 원문 그대로 표시합니다. 변경 사항과 알레르기는 학교 공지를 확인하세요.':(offset?'내일':'오늘')+' 등록된 급식 정보가 없어요.';
