...
import {MODEL,USER_DAILY,SITE_DAILY,SITE_MONTHLY,koreaDay,tutorInput,answerSchema,validAnswer} from '../server/tutor-policy.js';
...
export default async function handler(req,res){let generationId;
 try{
  ...
  const enabled=process.env.AI_TUTOR_ENABLED==='true';
  if(req.method==='GET'){
   const id=url.searchParams.get('id');
   if(id){if(!/^[0-9a-f-]{36}$/.test(id))reject('답변 주소를 확인해 주세요.');const [g]=await sql`SELECT id,subject,mode,question,result,status,created_at FROM public.campus_generations WHERE id=${id} AND user_id=${user.id}`;if(!g)reject('내 계정의 답변을 찾지 못했어요.',404);return respond(res,200,g);}
   const history=await sql`SELECT id,subject,mode,question,status,created_at FROM public.campus_generations WHERE user_id=${user.id} ORDER BY created_at DESC LIMIT 30`;
  ...
  // No credit purchasing/top-up API exists in this application. Exhausted
  // credits fail closed. An unused account may activate its first free grant.
  const credits=await gateway.getCredits();
  if(Number(credits.balance)<0.1&&Number(credits.totalUsed)>0)reject('무료 AI 잔액이 부족하여 생성을 멈췄어요. 자동 충전하지 않습니다.',429);
  ...
  const result=await generateText({model:gateway(MODEL),maxOutputTokens:3000,maxRetries:0,abortSignal:AbortSignal.timeout(45000),
 ...
 }catch(error){console.warn('[tutor]',{name:error.name,code:error.code,status:error.status||error.statusCode,generationId:generationId||null});if(generationId)await db()`UPDATE public.campus_generations SET status='error' WHERE id=${generationId}`.catch(()=>{});if(!error.status){error=Object.assign(new Error('AI가 답변을 완료하지 못했어요. 무료 크레딧·연결 상태를 확인한 뒤 다시 시도해 주세요. 실패한 요청도 안전을 위해 횟수에 포함됩니다.'),{status:503});}fail(res,error);}
}
