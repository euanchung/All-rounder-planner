...
import {estimatedAiCost,tutorInput,tutorEnabled} from '../server/tutor-policy.js';
import {analyzeSchedule,scheduleFailure,SCHEDULE_VERSION,SCHEDULE_MODELS} from '../server/schedule-ai.js';
import {aiQuota} from '../server/ai-quota.js';
...
export default async function handler(req,res){let generationId,quota;
 try{
  ...
  quota=aiQuota(sql,user.id,'timetable');
  ...
  const result=await analyzeSchedule(input.image,{beforeAttempt:()=>quota.reserve(),onAttempt:attempt=>console.warn('[schedule-attempt]',{...attempt,generationId})});
  const usage=result.usage,cost=estimatedAiCost(result.model,usage);
 ...
 }catch(error){if(quota)await quota.refund().catch(()=>console.warn('[schedule-refund-failed]',{generationId}));console.warn('[schedule]',{name:error.name,status:error.status||error.statusCode,generationId:generationId||null});if(generationId)await db()`UPDATE public.campus_generations SET status='error' WHERE id=${generationId}`.catch(()=>{});fail(res,scheduleFailure(error));}
}
