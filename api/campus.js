...
import {effectiveRole,canPublish,requestedRoles} from '../src/roles.js';
...
export default async function handler(req,res){try{
 ...
 const role=effectiveRole(user,person,member),site=await siteSettings(sql);
 ...
 if(req.method==='GET'){
  ...
  if(rooms.length){const counts=await sql`SELECT m.room_id,count(*)::integer AS unread,max(m.created_at) AS latest FROM public.campus_messages m LEFT JOIN public.campus_reads r ON r.room_id=m.room_id AND r.user_id=${user.id} WHERE m.room_id=ANY(${rooms.map(r=>r.id)}::uuid[]) AND m.sender_id<>${user.id} AND m.created_at>COALESCE(r.read_at,'epoch'::timestamptz) AND NOT EXISTS(SELECT 1 FROM public.campus_blocks b WHERE (b.user_id=${user.id} AND b.blocked_id=m.sender_id) OR (b.blocked_id=${user.id} AND b.user_id=m.sender_id)) GROUP BY m.room_id`;for(const room of rooms)Object.assign(room,counts.find(c=>c.room_id===room.id)||{unread:0,latest:null});}
  ...
  return respond(res,200,{role,site,requestedRole:account?.requested_role||'student',person:person||null,member:member||null,notices,people,rooms,applications,reports});
 }
 ...
 if(action==='request-role'){
  if(!requestedRoles.includes(body.role))err('신청할 역할을 확인하세요.');
  await sql`INSERT INTO public.campus_accounts(user_id,requested_role) VALUES (${user.id},${body.role}) ON CONFLICT(user_id) DO UPDATE SET requested_role=EXCLUDED.requested_role`;
  if(body.role==='teacher'&&person&&person.teacher_status!=='approved')await sql`UPDATE public.campus_people SET teacher_status='pending' WHERE user_id=${user.id}`;
  ...
  if(member?.owner_id!==user.id&&user.role!=='admin')err('담당 선생님만 반장·부반장을 지정할 수 있어요.',403);const assigned=body.role|| (body.enabled===true?'leader':'student');if(!['student','leader','deputy'].includes(assigned)||!member)err('역할과 학급을 확인하세요.');await sql`UPDATE public.campus_members SET role=${assigned} WHERE class_id=${member.id} AND user_id=${str(body.userId)} AND role<>'teacher'`;
 }else if(action==='notice'){
  if(!member||!canPublish(role))err('선생님·반장·부반장만 공지를 게시할 수 있어요.',403);if(!site.notices_enabled&&user.role!=='admin')err('관리자가 공지 게시를 잠시 중지했어요.',403);await sql`INSERT INTO public.campus_notices(id,class_id,author_id,title,body) VALUES (${randomUUID()},${member.id},${user.id},${str(body.title,200)},${str(body.text,12000)})`;await notifyPeers(sql,user.id,null,member.id).catch(()=>console.warn('[push]',{status:503}));
 }else if(action==='timetable'){
  if(!member||!['teacher','admin'].includes(role))err('승인된 선생님만 학급 시간표를 편집할 수 있어요.',403);if(!validDate(body.week)||!validTable(body.table)||!validPeriodTimes(body.periodTimes)||(body.daySchedule!==undefined&&!validDaySchedule(body.daySchedule))||(body.examMode!==undefined&&typeof body.examMode!=='boolean'))err('시간표와 교시 시각을 확인해 주세요.');await sql`INSERT INTO public.campus_tables(class_id,week,payload) VALUES (${member.id},${body.week},${JSON.stringify({table:body.table,periodTimes:body.periodTimes,...(body.daySchedule?{daySchedule:body.daySchedule,examMode:!!body.examMode}:{})})}::jsonb) ON CONFLICT(class_id,week) DO UPDATE SET payload=EXCLUDED.payload`;
 }else if(action==='room'){
  ...
  if(ids.length===1){const[existing]=await sql`SELECT id FROM public.campus_rooms WHERE auto_key IS NULL AND cardinality(members)=2 AND members @> ${[user.id,...ids]}::text[] ORDER BY created_at LIMIT 1`;if(existing)return respond(res,200,{id:existing.id});}const [count]=await sql`SELECT count(*)::integer AS n FROM public.campus_rooms WHERE ${user.id}=ANY(members)`;if(count.n>=50)err('대화방은 최대 50개입니다.');const id=randomUUID();await sql`INSERT INTO public.campus_rooms(id,name,members,created_by) VALUES (${id},${str(body.name,100)},${[user.id,...ids]}::text[],${user.id})`;return respond(res,200,{id});
 }else if(action==='message'){
  if(body.clientId!==undefined&&!uuid(body.clientId))err('메시지 식별자를 확인하세요.');if(body.clientId){const [sent]=await sql`SELECT id,room_id FROM public.campus_messages WHERE sender_id=${user.id} AND client_id=${body.clientId}`;if(sent){await roomAccess(sent.room_id);return respond(res,200,{ok:true,id:sent.id,replayed:true});}}
  if((body.taskId||body.sharedDraft)&&(!canPublish(role)||!member))err('할일 공유는 선생님·반장·부반장만 할 수 있어요.',403);
  if((body.taskId||body.sharedDraft)&&!site.sharing_enabled&&user.role!=='admin')err('관리자가 할일 공유를 잠시 중지했어요.',403);
  if(body.taskId||body.sharedDraft){await limit(sql,user.id,'share',5);const date=new Date(Date.now()+9*3600000).toISOString().slice(0,10);const [quota]=await sql`INSERT INTO public.campus_limits(bucket,hits,expires_at) VALUES (${'share-day:'+user.id+':'+date},1,now()+interval '2 days') ON CONFLICT(bucket) DO UPDATE SET hits=campus_limits.hits+1 RETURNING hits`;if(quota.hits>30)err('할일 공유는 하루 30회까지 가능해요.',429);}
  ...
  let shared=null;if(body.sharedDraft){if(role!=='teacher'&&role!=='admin')err('공유 할일 직접 등록은 선생님만 사용할 수 있어요.',403);const t=body.sharedDraft;if(typeof t.title!=='string'||!t.title.trim()||t.title.length>200||!validDate(t.due)||!Number.isInteger(t.minutes)||t.minutes<1||t.minutes>10080)err('공유 할일의 제목·마감·소요 시간을 확인하세요.');shared={title:t.title.trim(),fields:{due:t.due,time:null,subject:typeof t.subject==='string'?t.subject.slice(0,100):null},totalMinutes:t.minutes,category:'assignment',taskType:'assignment',priority:2,difficulty:3};}if(body.taskId){const [w]=await sql`SELECT data FROM public.radar_workspace WHERE user_id=${user.id}`;const t=w?.data.tasks.find(t=>t.id===body.taskId);if(!t)err('내 할일을 찾지 못했어요.');shared={title:t.title,fields:{due:t.fields.due,time:t.fields.time,subject:t.fields.subject},totalMinutes:t.totalMinutes??t.minutes,category:t.category,taskType:t.taskType||null,startDate:t.startDate||null,priority:t.priority,difficulty:t.difficulty||3};}if(!text&&!shared)err('메시지 또는 공유할 할일을 선택해 주세요.');const inserted=await sql`INSERT INTO public.campus_messages(id,room_id,sender_id,body,shared,client_id) VALUES (${randomUUID()},${room.id},${user.id},${text},${shared?JSON.stringify(shared):null}::jsonb,${body.clientId||null}) ON CONFLICT(sender_id,client_id) WHERE client_id IS NOT NULL DO NOTHING RETURNING id`;if(inserted.length)await notifyPeers(sql,user.id,room).catch(()=>console.warn('[push]',{status:503}));
 }else if(action==='read-room'){
  const room=await roomAccess(body.roomId);if(typeof body.through!=='string'||!Number.isFinite(Date.parse(body.through)))err('읽은 시각을 확인하세요.');const through=Date.parse(body.through)>Date.now()?new Date().toISOString():body.through;await sql`INSERT INTO public.campus_reads(user_id,room_id,read_at) VALUES(${user.id},${room.id},${through}) ON CONFLICT(user_id,room_id) DO UPDATE SET read_at=GREATEST(campus_reads.read_at,EXCLUDED.read_at)`;
...
}catch(error){console.warn('[campus]',{method:req.method,status:error.status||503,code:error.code||null});fail(res,error);}}
