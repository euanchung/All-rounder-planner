...
export default async function handler(req,res){try{
 ...
 if(action==='request-role'){
  ...
  if(!member||!canPublish(role))err('선생님·반장·부반장만 공지를 게시할 수 있어요.',403);if(!site.notices_enabled&&user.role!=='admin')err('관리자가 공지 게시를 잠시 중지했어요.',403);await sql`INSERT INTO public.campus_notices(id,class_id,author_id,title,body) VALUES (${randomUUID()},${member.id},${user.id},${str(body.title,200)},${str(body.text,12000)})`;
