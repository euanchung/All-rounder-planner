import {normalizeLanguage,matchWords} from './notice-language.js';
...
const dateToken='(?:20\\d{2}[-./년\\s]+)?\\d{1,2}(?:월|[/.])\\s*\\d{1,2}일?|오늘|내일|모레|글피|\\d{1,3}\\s*일\\s*(?:뒤|후)|(?:(?:이번|다음)\\s*주\\s*)?[월화수목금토일]요일';
...
function blocks(text, base) {
  ...
  for(const raw of text.split(/\r?\n/).map(s=>s.trim()).filter(Boolean)) {
    ...
    const header=normalizeLanguage(line).replace(/[\[\]:：]/g,'').trim();
...
export function inspectNotice(text,tasks,base=today()) {
  ...
  return parts.map(source=>{
    ...
    const warnings=[...new Set([...analyze(cleaned,base).issues,...parsed.warnings])].filter(s=>!s.startsWith('변경 설명')&&!s.startsWith('마감 미정'));
    if(/아니|제외|말고|하지\s*않|취소/.test(cleaned))warnings.push('부정·제외 표현이 있어요. 실제로 해야 하는 내용인지 확인해 주세요.');
    if(/취소/.test(source))warnings.push('취소 공지는 자동 삭제하지 않아요. 원래 할일에서 직접 완료·삭제를 결정하세요.');
    return {parsed,candidates,targetId,old:old?structuredClone(old):null,choice:clear?'update':candidates.length?'choose':'new',matchReason:clear?'과목·분류·제목이 기존 항목과 유사해요.':candidates.length?'유사한 제목을 찾았지만 하나로 확정할 근거가 부족해요.':'충분히 유사한 기존 항목을 찾지 못했어요.',warnings};
  });
}
...
export function applyNoticeRows(tasks, rows, idFactory, at=new Date().toISOString()) {
  ...
  for(const row of rows) {
    ...
    touched.add(old.id);
    if(!noticeChanges(row,old).length && (!row.editTitle||row.parsed.title===old.title) && (!row.editCategory||taskCategory(old)===row.parsed.category) && !row.warnings.length){skipped++;continue;}
