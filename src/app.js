...
function weekPanel(){const p=plan(data.tasks,capacities());return `<section class="panel week-panel"><div class="panel-heading"><h2>앞으로 7일 · 일일 시간 여유</h2><button class="text-button" data-action="view" data-view="planner">계획 / 시간 변경 ↗</button></div><div class="week-remaining">${p.days.slice(0,7).map(d=>`<button data-action="day" data-date="${d.date}"><span class="week-dayname">${['일','월','화','수','목','금','토'][new Date(d.date+'T12:00:00').getDay()]}</span><span class="week-date">${Number(d.date.slice(5,7))}.${Number(d.date.slice(8,10))}</span><strong>${Math.max(0,d.capacity-d.used)}<small>분</small></strong><small>여유 / 배치 가능 ${d.capacity}분</small></button>`).join('')}</div><p class="muted">일일 시간에서 진행 기록을 빼고, 오늘은 자정까지 실제 남은 분으로도 제한해요. 수업·휴식 시간대는 자동 파악하지 않으니 일일 시간을 조정해 주세요.</p></section>`;}
...
document.addEventListener('click',async event=>{
  ...
  if(action==='approve-plan'){data.acceptedPlan={at:new Date().toISOString(),start:today()};save();render();toast('계획을 승인했어요. 할일이나 일일 시간을 바꾸면 다시 확인받습니다.');}
...
document.addEventListener('input',event=>{if(event.target.closest('#source-form,#review-form,#settings-form,#notice-apply-form'))captureForms();});
document.addEventListener('change',event=>{if(event.target.closest('#source-form,#review-form,#settings-form,#notice-apply-form'))captureForms();});
render();
bootstrap();
...
function captureForms(){
  for(const id of ['source-form','review-form','settings-form','notice-apply-form']){
...
document.addEventListener('submit',event=>{
  ...
  try{
    ...
    if(f.id==='pin-form'){
      ...
      if(!t||t.done||!academic(t)||!validDate(date)||date<today()||!t.fields.due||date>t.fields.due||!Number.isInteger(minutes)||minutes<1)throw new Error('오늘부터 할일의 마감 사이 날짜와 양수 시간을 입력해 주세요. 마감 미정이면 먼저 마감을 정해 주세요.');
...
async function checkReminders(){
  ...
  try{if(localStorage.getItem(reminderKey())!=='enabled')return;const now=Date.now(),key=reminderKey()+'-sent',sent=JSON.parse(localStorage.getItem(key)||'{}');
    for(const t of data.tasks.filter(t=>!t.done&&t.fields.due)){const due=Date.parse(t.fields.due+'T'+(t.fields.time||'09:00')+':00'),tag=t.id+':'+due;if(due>=now&&due-now<=1800000&&!sent[tag]){await showReminder('곧 마감인 할일이 있어요. 메타시간표에서 확인하세요.');sent[tag]=now;}}
    for(const [k,n] of Object.entries(sent))if(now-n>172800000)delete sent[k];localStorage.setItem(key,JSON.stringify(sent));
    if(studyState.timer&&studyState.timer.endsAt<=now&&!studyState.timer.notified){await showReminder('5분이 지났어요. 실제로 진행한 시간을 기록해 보세요.');studyState.timer.notified=true;persistDraft();}
...
document.addEventListener('submit',event=>{
 ...
 try{
  ...
  const values=new FormData(event.target),rows=noticeState.rows.flatMap((r,i)=>{
   ...
   return [{...r,parsed:{...r.parsed,totalMinutes:n,minutesEstimated:n===original?r.parsed.minutesEstimated:false,fields:{...r.parsed.fields,due:due||null}}}];
  });
...
document.addEventListener('click',event=>{
 ...
 if(b.dataset.action==='notice-target'&&noticeState){
  ...
  delete forms['notice-apply-form'];render();
 }
});
