...
const capacities=()=>{const weekly=data.profile?.weeklyMinutes||Array(7).fill(data.capacity),date=today();const spent=data.tasks.reduce((n,t)=>n+(t.studyLog||[]).reduce((sum,l)=>sum+(localDate(l.at)===date?l.minutes:0),0),0);return {weekly,today:date,spent,overrides:data.profile?.dateOverrides||{}};};
...
function resetStudy(){quickState={text:'',batch:null,editId:'',forms:{},notes:{}};studyState={taskId:'',problem:1,forms:{},timer:null};mathResult=null;}
...
function dashboard(){const p=plan(data.tasks,capacities());return `${quickComposer(quickState,data.tasks)}${quickEditor(data.tasks.find(t=>t.id===quickState.editId))}${overloadPanel(data.tasks,capacities(),today())}${tasksPanel()}<section class="panel week-panel"><div class="panel-heading"><h2>앞으로 7일 · 남는 시간</h2><button class="text-button" data-action="view" data-view="planner">계획 / 시간 변경 ↗</button></div><div class="week-remaining">${p.days.slice(0,7).map(d=>`<button data-action="day" data-date="${d.date}" aria-label="${displayDate(d.date)} 남은 ${Math.max(0,d.capacity-d.used)}분, 가용 ${d.capacity}분"><span class="week-dayname">${['일','월','화','수','목','금','토'][new Date(d.date+'T12:00:00').getDay()]}</span><span class="week-date">${Number(d.date.slice(5,7))}.${Number(d.date.slice(8,10))}</span><strong>${Math.max(0,d.capacity-d.used)}<small>분</small></strong><small>남음 / 가용 ${d.capacity}분</small></button>`).join('')}</div></section><details class="extra-analysis"><summary>작업량·우선순위 자세히 보기</summary>${summary()}${priorityPanel(data.tasks,capacities(),today(),p.days[0])}</details>${demo?'<button class="text-button" data-action="sample-revision">물리 수정 공지 비교 예제 →</button>':''}`;}
...
document.addEventListener('submit',event=>{
  ...
  if(event.target.id==='review-form'){
    ...
    const task={...draft.old,id:draft.old?.id||crypto.randomUUID(),title,source:draft.source,sourceDate:draft.sourceDate,fields,minutes,totalMinutes,completedMinutes,priority,difficulty,problems:resizeProblems(draft.old?.problems,problemCount),studyLog:draft.old?.studyLog||[],done:draft.old?.done||false,history:draft.old?[...draft.old.history,{source:draft.old.source,sourceDate:draft.old.sourceDate,fields:draft.old.fields,minutes:draft.old.minutes,at:new Date().toLocaleString('ko-KR')}].slice(-100):[]};
...
function persistDraft(){if(booting||demo)return;try{sessionStorage.setItem(draftKey(),JSON.stringify({forms,draft,selected,studyState,quickState}));}catch{storageError='작성 중 초안을 임시 보관하지 못했습니다. 이 탭을 닫기 전에 내용을 복사해 주세요.';}}
function restoreDraft(){try{const p=JSON.parse(sessionStorage.getItem(draftKey())||'null');if(p&&typeof p.forms==='object'){forms=p.forms;if(p.quickState?.forms&&p.quickState?.notes)quickState=p.quickState;if(p.studyState?.forms&&typeof p.studyState.forms==='object')studyState=p.studyState;selected=typeof p.selected==='string'?p.selected:'';if(p.draft?.analysis&&typeof p.draft.source==='string'&&validDate(p.draft.sourceDate))draft=p.draft;}}catch{toast('이 탭의 초안을 읽지 못했습니다. 저장된 과제에는 영향이 없어요.');}}
...
function addQuickRows(rows){
  ...
  if(!unique.length)throw new Error('같은 제목·마감·분류의 할 일이 이미 있어요. 기존 할 일의 수정 버튼을 이용하세요.');
...
document.addEventListener('submit',event=>{
  ...
  try{
    if(f.id==='quick-form'){const rows=parseBatch(values.text,today());if(!rows.length)throw new Error('할 일을 한 줄 입력해 주세요.');if(rows.length===1)return addQuickRows(rows);quickState.batch=rows;render();return;}
    if(f.id==='batch-form'){
    ...
    if(f.id==='quick-edit-form'){
      ...
      if(!t)throw new Error('할 일을 다시 선택해 주세요.');
    ...
    if(f.id==='pin-form'){
      ...
      if(occupied+minutes>day.capacity)throw new Error('이 날의 가용 시간을 넘어요. 직접 계획을 줄이거나 날짜별 시간을 조정해 주세요.');
      t.planSlots=[...slots,{date,minutes}];delete quickState.forms[quickKey(f)];invalidate();render();return toast('직접 정한 시간을 먼저 확보했어요. 다른 할 일의 부족 경고도 확인해 주세요.');
    }
    if(f.id==='date-capacity-form'){
      ...
      if(value<0||value>720)throw new Error('변경 후 하루 총 가용 시간은 0~720분이어야 해요.');
      overrides[date]=value;if(Object.keys(overrides).length>366)throw new Error('날짜별 조정은 366개까지 보관할 수 있어요. 지난 조정을 해제해 주세요.');
      data.profile.dateOverrides=overrides;delete quickState.forms[quickKey(f)];invalidate();render();return toast(date+'의 총 가용 시간을 '+value+'분으로 바꿨어요.');
    }
...
document.addEventListener('click',async event=>{
  ...
  try{
    ...
    if(action==='calendar-export'){
      if(!data.tasks.some(t=>!t.done&&t.fields.due))return toast('마감일이 있는 진행 중 할 일을 먼저 추가해 주세요.');
      const url=URL.createObjectURL(new Blob([calendarFile(data.tasks)],{type:'text/calendar;charset=utf-8'})),a=document.createElement('a');a.href=url;a.download='meta-timetable.ics';a.click();setTimeout(()=>URL.revokeObjectURL(url),5000);toast('캘린더 앱에서 가져오세요. 개인 할 일 정보가 포함된 파일입니다.');
    }
...
async function checkReminders(){
  ...
  try{if(localStorage.getItem(reminderKey())!=='enabled')return;const now=Date.now(),key=reminderKey()+'-sent',sent=JSON.parse(localStorage.getItem(key)||'{}');
    for(const t of data.tasks.filter(t=>!t.done&&t.fields.due)){const due=Date.parse(t.fields.due+'T'+(t.fields.time||'09:00')+':00'),tag=t.id+':'+due;if(due>=now&&due-now<=1800000&&!sent[tag]){await showReminder('곧 마감인 할 일이 있어요. 메타시간표에서 확인하세요.');sent[tag]=now;}}
    for(const [k,n] of Object.entries(sent))if(now-n>172800000)delete sent[k];localStorage.setItem(key,JSON.stringify(sent));
    if(studyState.timer&&studyState.timer.endsAt<=now&&!studyState.timer.notified){await showReminder('5분이 지났어요. 실제 공부한 시간을 기록해 보세요.');studyState.timer.notified=true;persistDraft();}
...
document.addEventListener('submit',async event=>{
  ...
  if(id==='settings-form'){
    ...
    const profile={dateOverrides:data.profile?.dateOverrides||{},name:f.name.trim(),school:f.school.trim(),grade:f.grade.trim(),className:f.className.trim(),weeklyMinutes:Array.from({length:7},(_,d)=>Number(f['capacity-'+d])),timetable:Array.from({length:5},(_,d)=>Array.from({length:8},(_,i)=>f['slot-'+d+'-'+i].trim())),onboarded:true};
...
document.addEventListener('click',async event=>{
  ...
  try{
    ...
    if(action==='send-code'){button.disabled=true;await authAction('send-code',{});authMessage='발송 요청을 접수했어요. '+cloud.user.email+'의 스팸함도 확인하세요. 도착 여부는 아직 확인되지 않았으며, 최신 인증번호는 15분 안에 입력해 주세요.';render();}
    if(action==='logout'){
