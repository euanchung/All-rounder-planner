...
const capacities=()=>data.profile?.weeklyMinutes||data.capacity;
...
function changeDemo(){
  ...
  draft=null;forms={};view='dashboard';selected='';restoreDraft();render();toast(demo?'예제 공간입니다. 내 데이터는 바뀌지 않습니다.':'내 공간으로 돌아왔습니다.');
}
function nav(){return `<aside class="sidebar"><a href="#" class="brand" data-action="home"><span class="logo">◎</span><span>변생 레이더<small>NOTICE TO PLAN</small></span></a><div class="workspace-label">나의 워크스페이스</div><nav aria-label="주 메뉴">${[['dashboard','한눈에 보기'],['notice','공지 분석'],['planner','나의 계획'],['history','변경 기록'],['settings','개인 설정'],['account',cloud.user?'내 계정':'로그인 / 회원가입'],...(cloud.user?.role==='admin'?[['admin','관리자']]:[])].map(([key,label])=>`<button data-action="view" data-view="${key}" class="nav-item ${view===key?'active':''}" ${view===key?'aria-current="page"':''}><span aria-hidden="true">${icons[key]||'⚙'}</span>${label}${key==='notice'?'<b>＋</b>':''}</button>`).join('')}</nav><div class="sidebar-bottom"><div class="local-dot">${demo?'예제 체험 중':cloud.user?'개인별 클라우드 저장':'이 브라우저에 저장'}</div><p>${cloud.user?'내 과제와 설정은 계정별로 보관돼요.':'로그인하면 다른 기기에서도 내 계획을 이어 볼 수 있어요.'}</p><button class="text-button" data-action="export">백업 내려받기 ↗</button><label class="import-button" for="import">백업 불러오기<input id="import" type="file" accept=".json,application/json" hidden></label></div></aside>`;}
function heading(){const titles={dashboard:['한눈에 보기','바뀐 공지부터 오늘의 할 일까지.'],notice:['공지 분석','원문을 남기고, 바뀐 내용만 확인하세요.'],planner:['나의 계획','마감 전에 끝낼 수 있도록 시간을 나눠요.'],history:['변경 기록','어떤 공지에서 무엇이 달라졌는지 되짚어요.'],settings:['개인 설정','나의 요일, 나의 속도에 맞추세요.'],account:['내 계정','개인별로 안전하게 이어 쓰는 공간.'],admin:['관리자','개인 기록을 열람하지 않는 운영 현황.']};return `<header class="page-heading"><div><div class="eyebrow">${displayDate(today())} · ${demo?'예제 워크스페이스':'MY WORKSPACE'}</div><h1>${titles[view][0]}</h1><p>${titles[view][1]}</p></div><button class="button secondary" data-action="demo">${demo?'내 공간으로 돌아가기':'예제로 체험하기'} <span aria-hidden="true">↗</span></button></header>`;}
...
function render(){
  ...
  if(!['dashboard','notice','planner','history','settings','account','admin'].includes(view))view='dashboard';
...
document.addEventListener('change',event=>{
  ...
  if(event.target.id==='import'){
    ...
    file.text().then(text=>{const imported=JSON.parse(text);if(!validateBackup(imported))throw new Error();if(!confirm(`${demo?'예제':'현재'} 공간의 과제 ${data.tasks.length}개를 백업 내용으로 교체할까요? 먼저 백업을 내려받는 것을 권장합니다.`))return;data=upgrade({...imported,acceptedPlan:null});storageLocked=false;save();draft=null;forms={};selected='';view='dashboard';render();toast('백업을 불러왔어요.');}).catch(()=>toast('올바른 변생 레이더 백업 파일이 아닙니다. 현재 데이터는 그대로예요.'));
  }
});
...
function persistDraft(){if(booting||demo)return;try{sessionStorage.setItem(draftKey(),JSON.stringify({forms,draft,selected}));}catch{storageError='작성 중 초안을 임시 보관하지 못했습니다. 이 탭을 닫기 전에 내용을 복사해 주세요.';}}
function restoreDraft(){try{const p=JSON.parse(sessionStorage.getItem(draftKey())||'null');if(p&&typeof p.forms==='object'){forms=p.forms;selected=typeof p.selected==='string'?p.selected:'';if(p.draft?.analysis&&typeof p.draft.source==='string'&&validDate(p.draft.sourceDate))draft=p.draft;}}catch{toast('이 탭의 초안을 읽지 못했습니다. 저장된 과제에는 영향이 없어요.');}}
...
document.addEventListener('submit',async event=>{
  ...
  try{
    ...
    demo=false;realData=null;forms={};draft=null;selected='';storageError='';storageLocked=false;view='dashboard';window.history.replaceState({radar:true},'','#dashboard');await bootstrap();
...
document.addEventListener('click',async event=>{
  ...
  try{
    ...
    if(action==='logout'){
      ...
      draft=null;forms={};selected='';storageError='';storageLocked=false;demo=false;realData=null;view='account';render();
    }
