...
export function dailyCapacity(capacity,date){const week=capacity?.weekly||capacity;const base=capacity?.overrides?.[date]??(Array.isArray(week)?week[new Date(date+'T00:00:00Z').getUTCDay()]:week);return Math.max(0,(Number(base)||0)-(capacity?.today===date?capacity.spent:0));}
export function availableMinutes(capacity,start,count){
  ...
  if(capacity?.today&&daysLeft(capacity.today,start)>=0&&daysLeft(capacity.today,start)<count){const original=capacity?.overrides?.[capacity.today]??week[new Date(capacity.today+'T00:00:00Z').getUTCDay()];sum-=Math.min(original,capacity.spent);}
...
export function validStudyFields(t){
  ...
  if(t.category!==undefined&&!['study','bring','buy','other'].includes(t.category))return false;
