...
export function recordStudy(task,minutes,id,at){
  if(task.done||!Number.isInteger(minutes)||minutes<1||minutes>remainingMinutes(task))throw new Error('진행 시간은 1분 이상, 남은 시간 이하로 입력해 주세요.');
  if((task.studyLog?.length||0)>=200)throw new Error('할일당 진행 기록은 200회까지 보관할 수 있어요. 새 할일로 나누어 주세요.');
  const totalMinutes=task.totalMinutes??task.minutes,completedMinutes=(task.completedMinutes??0)+minutes;
  let consumed=minutes;
  const planSlots=(task.planSlots||[]).slice().sort((a,b)=>a.date.localeCompare(b.date)).map(s=>{const used=Math.min(consumed,s.minutes);consumed-=used;return {...s,minutes:s.minutes-used};}).filter(s=>s.minutes>0);
  return {...task,...(task.planSlots?{planSlots}:{}),totalMinutes,completedMinutes,minutes:totalMinutes-completedMinutes,studyLog:[...(task.studyLog||[]),{id,minutes,at}]};
}
export function undoStudy(task,id){
  const entry=task.studyLog?.find(l=>l.id===id);if(!entry)throw new Error('해당 기록을 찾지 못했습니다.');
  if(entry.minutes>(task.completedMinutes??0))throw new Error('완료한 시간이 수동 수정되어 이 기록을 취소할 수 없어요. 할일 수정에서 시간을 먼저 확인해 주세요.');
...
export function availableMinutes(capacity,start,count){
  ...
  for(const [date,n] of Object.entries(capacity?.overrides||{}))if(daysLeft(date,start)>=0&&daysLeft(date,start)<count)sum+=n-week[new Date(date+'T00:00:00Z').getUTCDay()];
  if(capacity?.today&&daysLeft(capacity.today,start)>=0&&daysLeft(capacity.today,start)<count){const original=capacity?.overrides?.[capacity.today]??week[new Date(capacity.today+'T00:00:00Z').getUTCDay()];sum-=original-dailyCapacity(capacity,capacity.today);}
...
export function validStudyFields(t){
  ...
  if(t.planSlots!==undefined&&(!Array.isArray(t.planSlots)||t.planSlots.length>100||new Set(t.planSlots.map(s=>s?.date)).size!==t.planSlots.length||!t.planSlots.every(s=>s&&/^\d{4}-\d{2}-\d{2}$/.test(s.date)&&Number.isFinite(Date.parse(s.date+'T00:00:00Z'))&&new Date(s.date+'T00:00:00Z').toISOString().slice(0,10)===s.date&&Number.isInteger(s.minutes)&&s.minutes>0&&s.minutes<=10080)))return false;
