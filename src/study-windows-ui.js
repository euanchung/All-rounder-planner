...
export function studyRows(day,rows){
 return '<input type="hidden" name="study-count-'+day+'" value="'+rows.length+'">'+rows.map((r,i)=>'<div class="study-time-row"><label>시작<input type="time" name="study-start-'+day+'-'+i+'" value="'+e(r.start)+'" required></label><span>~</span><label>종료<input type="time" name="study-end-'+day+'-'+i+'" value="'+e(r.end)+'" required></label><button type="button" class="button secondary" data-action="study-remove" data-day="'+day+'" data-index="'+i+'" aria-label="자습 구간 삭제">삭제</button></div>').join('');
}
export function studyEditor(week=emptyStudyWeek(),days=[0,1,2,3,4,5,6]){
 return '<div class="study-window-editor"><p class="muted">실제로 공부·과제에 쓸 수 있는 구간을 추가하세요. 여러 구간 가능 · 쉬는 날은 비워 두세요. 종료가 시작보다 이르면 다음날 종료(예: 23:40~00:50)입니다. 합계·남은 시간은 달력 날짜별로 계산하며 겹치는 구간은 저장할 수 없어요. 한국 시간 기준입니다.</p>'+days.map(d=>'<section class="study-window-day" data-day="'+d+'"><div class="panel-heading"><h3>'+('일월화수목금토'[d])+'요일</h3><button type="button" class="button secondary" data-action="study-add" data-day="'+d+'">＋ 구간 추가</button></div><div class="study-window-rows">'+studyRows(d,week[d]||[])+'</div><output class="study-window-total" aria-live="polite"></output></section>').join('')+'</div>';
}
export function studyDraft(values,day){
 const n=Math.max(0,Math.min(12,Number(values['study-count-'+day])||0));
 return Array.from({length:n},(_,i)=>({start:values['study-start-'+day+'-'+i]||'',end:values['study-end-'+day+'-'+i]||''}));
}
