...
export function matchWords(text){
 const normalized=normalizeLanguage(text).replace(/(?:제출|마감|기한|날짜|일정|소요|예상|시간|분량|안내|공지|변경|수정|연장|완료|까지)(?:일|이|은|는|을|를|됐어요|되었어요|됩니다|합니다|된|됨)?/g,' ').replace(/(?<![가-힣])(?:오늘|내일|모레|글피|[월화수목금토일]요일|이번|다음|주)(?![가-힣])|[\p{P}\p{S}]/gu,' ');
