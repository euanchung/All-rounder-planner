...
export const defaultAppearance=()=>({mode:'light',accent:'#4f46e5',background:null});
...
export function validAppearance(p){return !!p&&['light','dark','system'].includes(p.mode)&&validColor(p.accent)&&(p.background===null||validColor(p.background));}
...
export function applyAppearance(value){
 ...
 const vars={'--page':page,'--surface':surface,'--ink':surfaceInk,'--page-ink':ink,'--muted':mix(surfaceInk,surface,.22),'--accent':p.accent,'--accent-ink':contrastInk(p.accent),'--tint':tint,'--tint-ink':contrastInk(tint),'--border':mix(surface,surfaceInk,.27)};
