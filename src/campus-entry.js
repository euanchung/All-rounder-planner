...
export function commitEntries(original,entries,editId=null,idFactory=()=>crypto.randomUUID()){
 ...
 for(const [entryIndex,d] of entries.entries()){
  if(d.durationRequired)throw new Error((entryIndex+1)+'번 할일의 소요 시간을 직접 입력해 주세요.');
  ...
  const extras={taskType:d.taskType,startDate:d.startDate||null,deadlinePeriod:d.deadlinePeriod||null,deadlineAuto:!!d.deadlineAuto,priority:d.priority,difficulty:d.difficulty,note:d.note||'',completedMinutes:d.completedMinutes||0};
  ...
  else if(!tasks.some(t=>t.title===made.title&&t.fields.due===made.fields.due&&t.category===made.category&&t.fields.subject===made.fields.subject&&t.taskType===made.taskType))tasks.push({...made,...extras,minutes:d.totalMinutes-extras.completedMinutes});
 }
 return tasks;
}
