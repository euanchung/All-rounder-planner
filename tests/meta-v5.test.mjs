...
test('cancellation is warned and never deletes original',()=>{const t=old(),rows=inspectNotice('물리 보고서 취소',[t],base);assert.ok(rows[0].warnings.some(s=>s.includes('취소')));assert.equal(applyNoticeRows([t],rows,()=> 'b').tasks[0].done,false);});
...
test('same-day precise deadline caps plan and risk',()=>{const t=make('오늘 15시까지 물리 보고서 60분'),c=cap('14:30:00');assert.equal(taskMetrics(t,c,base).available,30);assert.equal(plan([t],c,base).totalShortage,30);assert.equal(taskMetrics(t,cap('15:00:00'),base).risk,'마감 지남');});
test('multiple timed deadlines accumulate required capacity',()=>{const a=make('오늘 15시까지 물리 보고서 30분'),b=make('오늘 15시까지 수학 숙제 30분','b'),c=cap('14:30:00');assert.equal(workloadSnapshot([a,b],c,base).maxShortage,30);assert.equal(plan([a,b],c,base).totalShortage,30);});
test('early manual deadline cannot exceed pre-deadline window',()=>{const t=make('오늘 15시까지 물리 보고서 60분');t.planSlots=[{date:base,minutes:60}];const p=plan([t],cap('14:30:00'),base);assert.equal(p.days[0].used,30);assert.equal(p.conflicts.length,1);});
