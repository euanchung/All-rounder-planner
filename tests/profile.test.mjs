...
test('weekly overload never assigns work to zero-capacity days',()=>{const p=plan([{id:'a',title:'task',minutes:120,priority:2,done:false,fields:{due:'2026-09-09'}}],[0,0,60,0,0,0,0],'2026-09-07');assert.equal(p.totalShortage,60);assert.equal(p.days[0].used,0);assert.equal(p.days[1].used,60);});
